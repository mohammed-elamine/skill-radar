    ...
    try:
        ...
        if result.success:
            if not quiet:
                ...
                click.echo(f"    Occ profile rows     : {result.occupation_profile_rows}")
                click.echo(f"    Skill profile rows   : {result.skill_profile_rows}")
                click.echo(f"    Occ similarity rows  : {result.occupation_similarity_rows}")
                click.echo(f"    Occ transition rows  : {result.occupation_transition_rows}")
                click.echo(f"    Run ID               : {result.run_id}")
                click.echo("")
            finalize_logging()
            sys.exit(EXIT_SUCCESS)
